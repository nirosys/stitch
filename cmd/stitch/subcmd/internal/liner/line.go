...
func (s *State) refreshSingleLine(prompt []rune, buf []rune, pos int) error {
	...
	if pLen+bLen < s.columns {
		if s.highlighter != nil {
			s, _ := s.highlighter.Highlight(string(buf))
			fmt.Print(s)
		} else {
...
func (s *State) refreshMultiLine(prompt []rune, buf []rune, pos int) error {
	...
	* going to the last row. */
	...
	output := string(buf)
	if s.highlighter != nil {
		output, _ = s.highlighter.Highlight(output)
	}
		...
		case rune:
			...
			default:
				if pos == len(line) && !s.multiLineMode &&
					...
					line = append(line, v)
					fmt.Printf("%c", v)
					if s.highlighter != nil { // Force refresh if we have a highlighter, so we get syntax highlighting in realtime
						s.needRefresh = true
					}
					pos++
				} else {
